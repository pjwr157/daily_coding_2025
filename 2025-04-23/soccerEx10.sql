...
SELECT * FROM tottenham_squad WHERE p_name LIKE '%손%'; #(問題4番まで)

SELECT * FROM tottenham_squad WHERE p_position = '공격수';
select * from tottenham_squad where p_position = '공격수'; #위아래 같음, 구분 타메니(問題5番から)

DELETE FROM tottenham_squad WHERE p_number = 7; #토트넘 7번 선수 삭제하기 (goodbye son)(問題6番まで)

UPDATE tottenham_squad SET p_name = '손손흥흥민민' where p_number=7;  #7번 선수 이름 변경 (welcome sonson)(問題7番まで)

select * from tottenham_squad where p_number >= 7; #7이상 선수들만 출전!(問題8番まで)

select * from tottenham_squad where p_name like '%제%' and p_number >= 6; #번호가 6이상이면서 이름에 '제' 이 들어간 선수들만 출력! (問題9番まで)

ALTER TABLE tottenham_squad ADD weekly_pay int;	#주급 추가! (問題10番まで)			
#ALTER TABLE tottenham_squad ADD weekly_pay int default 0;	#이렇게도 가능					
desc tottenham_squad; #확인 ( 테이블 구조 )			

select * from tottenham_squad order by p_number;					
select * from tottenham_squad order by p_number asc; #선수단 선수 목록을 p_number 순으로 정렬해서 보기 (問題11番まで)			
select * from tottenham_squad order by p_number desc;					
select * from tottenham_squad order by p_height desc, p_weight; #선수단 선수 목록을 키가 큰 선수부터 나오게 정렬하기, 그리고 같은 키, 몸무게가 가벼운 선수 먼저 나오게

update tottenham_squad set weekly_pay = 1000 where p_number=9; 
update tottenham_squad set weekly_pay = 2000 where p_number=14; #주급 정보 임의로 수정하기 (問題12番まで)
update tottenham_squad set weekly_pay = 1000000000 where p_number=7; #for you, son.
select * from tottenham_squad order by weekly_pay desc; #선수단 선수 목록을 주급 순으로 정렬해서 보기

select p_number,p_name from tottenham_squad; #선수단 정보 중 등번호, 이름만 뽑아보기 (問題16番まで)
select p_number as 등번호,p_name as 선수명 from tottenham_squad; #as == 이리이리 불러주이소~
# p_no int primary key auto_increment
# ALTER TABLE tottenham_squad ADD p_no INT PRIMARY KEY AUTO_INCREMENT FIRST;

select * from tottenham_squad; # Print the Table
select count(*) from tottenham_squad; #Gives the number
drop table tottenham_squad; #Erases the Table itself