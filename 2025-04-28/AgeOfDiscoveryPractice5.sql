...
-- 🛠️🛠️🛠️🛠️🛠️ SELECT 특훈 10문제 🛠️🛠️🛠️🛠️🛠️ (Study Here & Scroll down for Update Practice!)

select 도시명 from 도시;
SELECT 항해사명, 직급 FROM 항해사;
SELECT 선박명, 선박종류 FROM 선박;
SELECT 교역품명, 기준가격 FROM 교역품;
SELECT 항해사명 FROM 항해사 WHERE 직급 = '선장';
SELECT 선박명 FROM 선박 WHERE 현재선원수 >= 50;
SELECT 교역품명 FROM 교역품 WHERE 기준가격 >= 1000;
SELECT 항해사명 FROM 항해사 WHERE 도시ID = (SELECT 도시ID FROM 도시 WHERE 도시명 = '리스본');				-- 서브쿼리는 "값을 하나 뽑아서 비교"하는 방식이고,
SELECT 항해사명 FROM 항해사 WHERE 소속길드ID = (SELECT 길드ID FROM 길드 WHERE 길드명 = '고양이길드');			-- JOIN은 "테이블끼리 연결해서 한 번에 조회"하는 방식!
SELECT DISTINCT 도시명 FROM 도시 JOIN 도시교역품 ON 도시.도시ID = 도시교역품.도시ID JOIN 교역품 ON 도시교역품.교역품ID = 교역품.교역품ID WHERE 교역품명 = '와인';
...
UPDATE 보물 SET 위도 = 38.716900, 경도 = -9.139900 WHERE 건물명 = '조선소';
...
UPDATE 항해사 SET 직급 = '선장' WHERE 항해사명 = '김항해사';			-- 항해사 이름이 '김항해사'인 항해사의 직급을 '선장'으로 변경
